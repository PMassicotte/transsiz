...
# df2 <- df %>% 
#   group_by(data_source, station, hour, depth) %>% 
#   summarise(par_z_variable_transmittance = mean(par_z_variable_transmittance), n = n())
...
df %>% 
  ...
  scale_color_manual(
    breaks = c("pp_mixing_model.rov", "pp_under_ice.rov", "pp_mixing_model.suit", "pp_under_ice.suit"),
    values = RColorBrewer::brewer.pal(4, "Dark2"),
    labels = c(
      bquote(PP[Mixing]^{ROV}),
      bquote(PP[Underice]^{ROV}),
      bquote(PP[Mixing]^{SUIT}),
      bquote(PP[Underice]^{SUIT})
    )
  ) +
